...
_Інтерполяція рядків_ є способом створити новий рядок `String` із комбінації констант, змінних, літералів та виразів, включаючи їх значення всередині рядкового літералу. Кожен елемент, що вставляється в рядковий літерал, має бути оточений парою дужок, перед якими йде зворотній слеш \(`\`\):
...
Кожен рядок `String` має асоційований _тип індекса_, `String.Index`, котрий відповідає позиції кожного символа `Character` в рядку.
