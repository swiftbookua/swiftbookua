...
> > The `print(_:separator:terminator:)` function doesn’t have a label for its first argument, and its other arguments are optional because they have a default value. These variations on function syntax are discussed below in [Function Argument Labels and Parameter Names](function-argument-labels-and-parameter-names) and [Значення параметрів за замовчуванням](значення-параметрів-за-замовчуванням).
