...
Swift перетворює блок `if`-`else` на виклики методів `buildEither(first:)` та `buildEither(second:)`. Хоч ви й не викликаєте ці методи у вашому власному коді, приклад результату трансформації вище дозволяє легше побачити, як Swift трансформує ваш код при використанні синтаксису `DrawingBuilder`.
