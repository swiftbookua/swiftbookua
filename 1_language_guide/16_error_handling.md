...
У прикладі вище, функція `buyFavoriteSnack(person:vendingMachine:)` викликається через вираз `try`, оскільки вона може викинути помилку. Якщо викидається помилка, виконання одразу переноситься до пунктів `catch`, які почергово звіряють помилку зі своїми шаблонами і зупиняються при першому співпадінні. Якщо жоден із шаблонів не співпав, помилка обробляється останнім пунктом `catch`, і прив'язується до локальної константи `error`. Якщо не викидається жодної помилки, виконується решта інструкцій усередині інструкції `do`. 
...
Відловити декілька пов'язаних помилок можна також іншим способом: просто перечисливши їх після `catch`, розділяючи комами. Наприклад:
