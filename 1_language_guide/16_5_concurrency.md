...
Як і класи, актори є типами-посиланнями, тому порівняння типів-значень та типів-посилань у розділі [Класи як типи-посилання]({% link _book/1_language_guide/8_classes_and_structures.mdк%}#класи-як-типи-посилання) є справедливим і для них. На відміну від класів, актори дозволяють звертатись до їх змінюваного стану лише одній задачі за раз, що дозволяє для коду в кількох задачах взаємодіяти з одним і тим же екземпляром актору безпечно. Наприклад, ось актор, що записує значення вимірювання температури:
